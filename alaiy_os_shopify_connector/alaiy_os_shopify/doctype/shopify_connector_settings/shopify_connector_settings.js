frappe.ui.form.on("Shopify Connector Settings", {
	refresh(frm) {
		...
		frm.add_custom_button(__("Sync to Shopify Inventory"), () => {
			frappe.call({
				method: "alaiy_os_shopify_connector.api.sync.trigger_inventory_push",
				callback: () => frappe.show_alert({ message: __("Inventory push queued"), indicator: "blue" }, 5),
			});
		}, __("Actions"));

		frm.add_custom_button(__("Sync Orders from Shopify"), () => {
			frappe.call({
				method: "alaiy_os_shopify_connector.api.sync.trigger_orders_sync",
				callback: () => frappe.show_alert({ message: __("Order sync queued"), indicator: "blue" }, 5),
			});
		}, __("Actions"));
	},
});
