"""
Alaiy OS -> Shopify fulfillment push (outbound direction). The reverse
(delivery_notes.py) already pulls Shopify fulfillments IN as Delivery Notes;
nothing pushed a Delivery Note created here back OUT to Shopify -- create,
tracking edits, and cancel were all a ground-up gap. This module builds all
three.
...
def _match_dn_items_to_fulfillment_orders(dn, open_by_fulfillment_order):
    ...
    item it corresponds to (by variant id first, SKU as fallback -- same
    order push.py's own push_order_create already uses), and caps the
    quantity requested at the smaller of what the DN actually shipped and
    what Shopify still has open. Only lines the DN really has get pushed --
    this is what makes partial/warehouse-scanned shipments correct instead
    of blindly fulfilling everything still open on the order.
...
def push_delivery_note_fulfillment(delivery_note: str, tracking_number: str = None, carrier: str = None, tracking_url: str = None, notify_customer: bool = True):
    ...
    if fulfillment_ids:
        ...
        frappe.db.set_value("Delivery Note", dn.name, "sh_shopify_fulfillment_id", fulfillment_ids[0])
...
def push_delivery_note_fulfillment_job(delivery_note: str):
    ...
    try:
        push_delivery_note_fulfillment(
            delivery_note, tracking_number=dn.sh_tracking_number or None, carrier=dn.sh_carrier or None,
        )
    except Exception:
        frappe.log_error(
...
def on_delivery_note_update_after_submit(doc, method=None):
    """Editing sh_carrier/sh_tracking_number on a Delivery Note that already
    has a linked Shopify fulfillment pushes the change to Shopify."""
    if not doc.sh_shopify_fulfillment_id:
        return
    if not (doc.has_value_changed("sh_tracking_number") or doc.has_value_changed("sh_carrier")):
        return
    ...
    frappe.enqueue(
        ...
        tracking_number=doc.sh_tracking_number, carrier=doc.sh_carrier or "",
