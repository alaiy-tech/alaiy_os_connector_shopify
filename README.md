...
- **[Agents](docs/agents.md)** — the read-only question-answering pack this connector registers (17 tools, reaches Ask Alaiy), and the `listing_channels` adapter that gives `alaiy_os_agents`'s listing agent Shopify's fields, rules and validator; plus the roles and how to switch it all on.
