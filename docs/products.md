...
- **Never wipes** — the first-run auto-wipe this used to do as a "safety net against duplicates" was removed: it was the exact logic that once emptied real stock data live when a scheduler fired mid-wipe, and a brand-new site's first import simply creates everything fresh with nothing to skip, so there was never a real need to wipe first.
- **Paginated fetch** — `_PRODUCTS_QUERY` via the GraphQL client's cursor pagination.
- **Per product** (`_import_product`), on every run:
...
- **Six checkboxes** in Shopify Connection choose which statuses each direction (import/export) moves, all on by default. A status excluded from import is counted as skipped-with-reason in the sync log, not silently dropped.
