...
Gated by `Shopify Connection.sh_fulfillment_sync_direction` — **"Shopify → Alaiy OS (default)"** leaves this section's behavior fully inert; existing installs see no change. Switching to **"Alaiy OS → Shopify (two-way)"** activates it for sites where the warehouse ships out of Alaiy OS itself (e.g. scanning items on a Delivery Note against a Sales Order).
...
- `fulfillmentCreate`'s real input shape wraps every open fulfillment order's line items into one `lineItemsByFulfillmentOrder` array on a single call — `FulfillmentInput` has no `fulfillmentOrderId`/`fulfillmentOrderLineItems` fields of its own (confirmed live: "Field is not defined on FulfillmentInput"), so this is one API call total, not one per fulfillment order. When a DN's items genuinely span more than one open fulfillment order (rare — split across Shopify locations), Shopify creates a Fulfillment per order but the mutation only returns one; only that one is linked via `sh_shopify_fulfillment_id`, the rest are logged so it's visible rather than silently dropped.
- `sh_shopify_fulfillment_id` is always stored as the **plain legacy numeric id**, matching every other `sh_shopify_*_id` field in this app and the inbound path's own convention (`_sync_tracking` writes the same plain id). Storing the full GID here instead broke the very next webhook echo — confirmed live. The GID form (`gid://shopify/Fulfillment/...`) is rebuilt only at the point of calling a mutation that requires it (`_fulfillment_gid`).
...
- `push_fulfillment_for_delivery_note` — a separate whitelisted entry point, independent of the setting, for carrier connectors (e.g. FedEx's `create_shipment_for_delivery_note`) to call explicitly once they have a real tracking number. Routes by whether a fulfillment already exists on the Delivery Note: no fulfillment yet → creates one (same path as the submit hook, above); already linked → routes to `fulfillmentTrackingInfoUpdate` instead of a second create — confirmed live that a DN already pushed by the two-way submit hook has `sh_shopify_fulfillment_id` set by the time a carrier connector calls this with a real tracking number, and the create-only path threw outright on the second call.
