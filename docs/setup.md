...
| Import Products from Shopify | `api.sync.trigger_product_import` | Never wipes: creates new products, updates changed ones, skips unchanged ones, every run including the first. See [Products](products.md). |
